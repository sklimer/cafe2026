...
const TelegramStatus: React.FC = () => {
  ...
  // Check if status panel should be shown (based on environment variable)
  const showStatusPanel = import.meta.env.VITE_SHOW_TELEGRAM_STATUS !== 'false';
  ...
  return (
    <div className={`${isTelegramWebApp ? 'fixed top-0 left-0 right-0 bg-blue-100 border-b border-blue-300 p-2 text-xs z-50 overflow-y-auto max-h-40' : 'hidden'}`}>
      <details className="cursor-pointer">
        <summary className="font-bold mb-1 hover:bg-blue-200 rounded px-1">Telegram Mini App Status (click to expand)</summary>
